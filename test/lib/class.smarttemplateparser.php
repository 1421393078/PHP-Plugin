	...
	class SmartTemplateParser
	{
		...
		function SmartTemplateParser ( $template_filename )
		{
			...
			if ($hd = @fopen($template_filename, "r"))
			{
				if (filesize($template_filename))
				{
			        ...
			        $this->template = str_replace('%7B', '{', $this->template);
			        $this->template = str_replace('%7D', '}', $this->template);
				}
				else
				{
		...
		*/
		function compile( $compiled_template_filename = '' )
		{
			...
			//	'IF nnn=mmm' Blocks
			if (preg_match_all('/<!-- (ELSE)?IF ([a-zA-Z0-9_.]+)[ ]*([!=<>&\x7c]+)[ ]*(["]?[^"^\n]*["]?) -->/', $page, $var))
			{
			...
			if (preg_match_all('/{([a-zA-Z0-9_]+):([^}]*)}/', $page, $var))
			{
				foreach ($var[2] as $cnt => $tag)
				{
					...
					{
						$header  .=  "include_once \"".LIB_PATH."smarttemplate/smarttemplate_extensions/smarttemplate_extension_$extension.php\";\n";
